...
.multiselect-dropdown span.maxselected {width:100%; text-align: center;}
...
function MultiselectDropdown(options){
  ...
  document.querySelectorAll("select[multiple]").forEach((el,k)=>{
    ...
    var div=newEl('div',{class:'multiselect-dropdown',style:{width:'100%',padding:config.style?.padding??'',height:'50px'}});
    ...
    el.loadOptions=()=>{
      ...
      div.refresh=()=>{
        ...
        if(sels.length>(el.attributes['multiselect-max-items']?.value??5)){
          div.appendChild(newEl('span',{class:['optext','maxselected'],text:sels.length+' '+config.txtSelected,text_align:'center'}));          
        }
        else{
          sels.map(x=>{
